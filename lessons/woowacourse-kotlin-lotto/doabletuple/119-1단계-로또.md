...
리뷰어: "*Lotto 객체를 만들 때 setOf().map{}.toSet()하는 게 반복*되는데 *secondary constructor를 만들어서 조금 더 간편하게 Lotto 객체를 만들 수* 있게 하면 어떨까요? `Lotto(1, 2, 3, 4, 5, 6)`" — PR #60, #14 동일 패턴.
...
리뷰어: "*View에서 profitRate가 1보다 작은지, 큰지, 같은지와 같은 로직이 있는 것 좋지 않아 보여요*! *View가 아닌 다른 곳에서 profitRate를 가지고 enum으로 변환하고 View에서는 enum에 따라 message만 출력*해 주면 어떨까요?" — PR #148 (KwonDae) `Profit enum` 동일.
...
- **`!in` 연산자 + `in` 연산자** — Kotlin idiom (PR #14, #60 동일).
...
- **리뷰어의 *자기 정정*** — `프로프 ratecate가 이미 수익률을 나타내는 것이었네요`. *권위에 의존*하지 않는 *대화*.
