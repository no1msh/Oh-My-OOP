...
> lotto 미션 *최신 PR*. 말리빈의 *비유 + 메타-질문* 스타일.
...
리뷰어 (반복): "*어떤 값이 들어왔는지 에러 메시지에 담아 보는 건* 어떨까요? *어떤 장점이 생길 수 있을까요*?" + "*어떤 입력 값이었는지도 메시지에 남겨보는 건* 어떨까요?" — PR #21, #60 동일.
...
리뷰어: "*assertAll로 묶는 것과 그렇지 않은 것은 어떤 차이가 있을까요*?" — PR #31 (말리빈) 동일.

### 7. *일급 컬렉션 부재* (Lottos, LottoResults)

리뷰어 (반복): "*로또들을 담는 일급 컬렉션을 만들어 보는 건* 어떨까요?" + "*로또 결과들을 담는 일급 컬렉션을 만들어 보는 건* 어떨까요?" + 추가: "*여전히 유효한 피드백*이에요. *이번 미션의 요구사항이기도 한 일급 컬렉션을 만들어 보세요*." — *명세 요구사항*.
...
리뷰어 (*인용 가치 매우 높음*): "이렇게 되면 *수동 티켓을 만들 때마다 LottoFactory 객체와 ManualLottoGenerator 객체를 새로 생성*해야겠어요. *여기에서 자동, 수동 티켓을 만드는 역할을 수행하고, 랜덤의 요소를 국소적으로 분리해 보는 건 어떨까요? LottoGenerator 인터페이스가 반드시 수동 자동 구현체를 만들어야 한다는 것에 매몰되지 않아도 좋겠*어요." — *추상화의 자유*. *수동/자동 = LottoGenerator 구현체*에 강박 X.
...
리뷰어: "*테스트에 필요한 정보를 로직을 통해 만들어 내지 않고, 직접 하드코딩으로 생성*해 보는 건 어떨까요?" — PR #137 (hyemdooly) 동일.

### 12. *오해 여지 이름 vs 본인 기준*

리뷰어: "*이름에 오해의 여지가 있으면 피하는 게 좋겠지요. 그렇다고 무조건 피하는 것은 좋은 방법일지도 생각*해 보세요 :) *본인만의 기준을 만들어 낼 수 있는 좋은 고민거리*네요!" — PR #75 (말리빈) 동일 *기준과 근거*.
...
- **`Lotto` *당첨 무관*** — 당첨은 *WinningLotto* 책임. PR #60, #14 동일.
- **부 생성자 vs 팩토리 *의도 정리*** — *생성자 = 단순 생성*, *팩토리 = 분기/변환*. PR #90 (Gyuil-Hwnag) 동일.
- **에러 메시지에 *입력값*** = *디버깅 비용 절감*. *로그 수집 시대*에 *유일한 데이터 보존*. PR #21, #60 동일.
- **`assertAll` *Soft Assertion*** = 한 검증 실패해도 나머지 실행. PR #31 동일.
- **일급 컬렉션 *요구사항*** — *역할/책임 부여*가 진짜 가치 (PR #80, #119 동일).
...
- **이름 *오해 여지 vs 자기 기준*** — *완전 회피 X*. *왜 그 이름인지 근거*가 더 중요. PR #75 *기준과 근거* 동일 강사.
- **PR #153 = 최신 lotto PR** — 미션이 *지속*되고 있음.
